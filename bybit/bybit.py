...
coins={"USD","ETH","BTC","USDT","EOS"}

for cur in curs:
    for coin in coins:
        symbol = cur+coin
        #url = 'https://api.bybit.com/v2/public/kline/list?symbol='+symbol+'&interval=720&from=1546300800'
        #url='https://api.bybit.com/v2/public/kline/list?symbol='+symbol+'&interval=720&from=1561939200'
        url='https://api.bybit.com/v2/public/kline/list?symbol='+symbol+'&interval=720&from=1577836800'

        print(url)
        resp = requests.get(url)
        resp_json = resp.json()

        ret_code = resp_json['ret_code']

        if ret_code == 0:
            data = resp_json['result']
            df = pd.DataFrame(data)
            df['market_name'] = symbol
            #df.to_csv(symbol + '201901.csv')
           # df.to_csv(symbol + '201907.csv')
            df.to_csv(symbol + '202001.csv')
